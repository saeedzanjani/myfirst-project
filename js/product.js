...
const backDrop = document.querySelector('.cart-overlay')
...
class Veiw {
    displayProducts(products) {
        ...
        products.forEach((item) => {
            ...
            <h3>نام محصول: ${item.title}</h3>
            <h4>قیمت: ${item.price}</h4>
          </article>
            `
        })
    ...
    addCartItem(item) {
        ...
        <p class="remove-item">${item.amount}</p>
    ...
    cartProcess() {
        ...
        cartContent.addEventListener('click', (event)=>{
            ...
            if(event.target.classList.contains('fa-chevron-down')) {
                let loseAmount = event.target
                let id = loseAmount.dataset.id
                ...
                if(product.amount > 0){
                    Storage.saveCart(cart)
                    this.setCartValues(cart)
                    loseAmount.previousElementSibling.innerText = product.amount
                }else {
                    cartContent.removeChild(loseAmount.parentElement.parentElement)
                    this.removeCartItem(id)
                }
